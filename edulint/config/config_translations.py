...
CONFIG_TRANSLATIONS: Dict[str, Translation] = {
    ...
    # ),
    Option.ALLOWED_ONECHAR_NAMES.to_name(): Translation(
        Linter.PYLINT, ["--bad-names-rgxs=^[a-z]$"]
    )
}


def get_config_translations(config_file) -> Dict[str, Translation]:
    return {
        **CONFIG_TRANSLATIONS,
        **{
            group["name"]: Translation(Linter.from_name(group["for"], group["value"]))
            for group in config_file["translations"]
        },
    }
